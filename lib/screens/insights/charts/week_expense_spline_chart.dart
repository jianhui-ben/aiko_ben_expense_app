...
class WeekExpenseSplineChart extends StatefulWidget {
  final List<Transaction> transactions;
  const WeekExpenseSplineChart({super.key, required this.transactions});

  @override
  State<WeekExpenseSplineChart> createState() => _WeekExpenseSplineChartState();
}

class _WeekExpenseSplineChartState extends State<WeekExpenseSplineChart> {
...
List<_ChartData> convertTransactionsToChartData(List<Transaction> transactions, bool isEveryDayShow) {
  ...
  if (isEveryDayShow) {
    ...
    for (int i = 0; i < 7; i++) {
      final date = DateTime(currentDate.year, currentDate.month, currentDate.day - i);
